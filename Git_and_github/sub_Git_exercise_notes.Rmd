...
Now **push** your changes to Github using this button:  
...
* Right-click the file and select "Reset"   
    - note that the term 'reset' in this menu is quite sloppy, as reset is also a quite different command   
...
<br>
Your last commit ("change 3") shows the deleted parts in red and the new parts in green, so the commit changed `col = "blue"` to `col = "brown"`    ![](Git_exercise_notes_images/git_revert_command_1.JPG)  

<br><br>
The `git revert`command above changed `col = "brown"`  back to `col = "blue"` , and automatically makes the commit message `Revert change 3`     ![](Git_exercise_notes_images/git_revert_command_2.JPG)
