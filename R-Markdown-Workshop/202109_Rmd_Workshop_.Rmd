...
To create an R Mardown file click on File &rarr; New File &rarr; R Markdown and
...
"Markdown Quick Reference" under the <kbd>Help</kbd> field in the
...
The <code>&#96;&#96;&#96;</code> symbol on the top and bottom creates the chunk. `r` means the language you are coding on. Then you have the name of the chunk. This is optional, but **_you cannot have chunks with the same name!_**
...
In a markdown file, Control + Shift + O ('O' as in Oslo, not as in zero) will show you a nice 'clickable' outline in the right side of the script window, based on the #, ##, ###, and #### headings. You can also open the outline using the menu: Code > Show Document Outline. Note that you should only use # in the start of the document, otherwise the outline doesn't show properly.


![](images/Rmd_outline.png)

The Document Outline can also be used for ordinary R scripts; it will then show comments (lines starting with #) that ends with ---- (yes, four hyphens), for instance # Reading data ----.
### Challenge 2
...
In a markdown file, Control + Shift + O ('O' as in Oslo, not as in zero) will show you a nice 'clickable' outline in the right side of the script window, based on the #, ##, ###, and #### headings. You can also open the outline using the menu: Code > Show Document Outline. Note that you should only use # in the start of the document, otherwise the outline doesn't show properly.
![](images/Rmd_outline.png)
(Image file has been added to 'images')
The Document Outline can also be used for ordinary R scripts; it will then show comments (lines starting with #) that ends with ---- (yes, four hyphens), for instance # Reading data ----.
...
the <kbd>R Markdown Cheat Sheet</kbd> under the <kbd>Cheatsheets</kbd> section
of the <kbd>Help</kbd> field in the toolbar at the top of RStudio.
...
Try executing this chunk by clicking the <kbd>Run</kbd> button within the chunk or by placing your cursor inside it and pressing <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>I</kbd> on Windows and Linux, or <kbd>Cmd</kbd>+<kbd>Option</kbd>+<kbd>I</kbd> on Mac.. 
...
There are **7** errors in this document, **can you find and correct them?*
```
<pre>
&#96;&#96;&#96;{r cars echo=FALSE}
...
`4 - missing an asteristik on the question above`
...
`5 - there is a comma missing between the chunk title and parameter above`
...
`6- A closing grave accent is missing after the word FALSE in the text above. This is used to create code lines in text`
